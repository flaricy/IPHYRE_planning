...
class Node():
    def __init__(self, demo: IPHYRE, depth, idx, max_simu, parent = None, action = None, terminal = False) -> None:
        ...
        if len(self.demo.get_action_space_by_property()) == 0: # no objects can be eliminated
            self.terminal = True
    ...
    def rullout(self):
        ...
        '''
        if self.terminal:
            raise Exception('Already terminaled. Rullout banned.')
...
class TreeSearch():
    ...
    '''
    def __init__(self, demo: IPHYRE, max_depth = 6, timestep = 3/60, init_idx = 1, max_simu = 3000) -> None:
        demo.reset()
    ...
    def build_tree(self, max_rullout_node = 1 << 12):
        '''
        ...
        while self.ptr < len(self.node_list) and self.node_list[self.ptr].depth < self.max_depth:
            ...
            for _ , pos_x, pos_y in actions:
                print(f'\nRollout: {self.current_idx + 1}, current action: {pos_x}, {pos_y}')
                pos = (pos_x, pos_y)
